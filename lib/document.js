...
(function(){
  ...
  var BLANK = '__FIXME__'
    , TEMPLATE_PATH = __dirname + '/templates/schema.hbs';
  ...
  function generate(context) {
    ...
    return new Promise(function (resolve, reject) {
      try {
        ...
        self._execute(prepared, resolve);
      } catch (e) {
        reject(e);
      }
    });
  ...
  function _prepare(context) {
    var operation, response, result;
    result = _.map(context.getAgendaData(), function(agenda, index) {
      operation = _.findKey(context.named, function(value) {
        return value === index;
      });
      response = context.outcome.getData(index).actualResponse;
      return {
        operation: operation ? _.capitalize(operation) : BLANK
        , req: {
  ...
  function _execute(prepared, callback) {
    this.resource.context = prepared;

    var resource = this.resource
      , filename = (this.resource.name ? this.resource.name : BLANK) + '.md'
